...
def fitness_hit_part(output, correct_answer, hitler_seat):
    error_hit = 2/5 * ((output[hitler_seat] - correct_answer[hitler_seat]) ** 2)
    error_all = 3/5 * sum([abs(out) ** 2 for out in output[:hitler_seat] + output[hitler_seat + 1:]])
    return (error_all/7) + error_hit
...
def eval_genome(genome, config):
    ...
    for game in all_games[:training_games]:
        ...
        for turn in game[0]:
            ...
            error += fitness_hit_part(output,correct_answer,hitler_seat)
            turns += 1
...
def run(restore: bool, generations: int, prefix='neat-checkpoint-', file_name=''):
    ...
    for game in sample:
        ...
        for turn in game[0]:
            ...
            # Make memory copy of the output so you can use it as input on next turn.
            memory = output.copy()
            if output.index(max(output)) == hitler_seat and hitler_seat > 1/6:
                correct_h += 1
            # Product of lib,hit error is divided by 7 and subtracted from 1. It is added to error
            correct_t += 7 - sum(round(abs(out-real)) for out,real in zip(output,correct_answer))
            print(7- sum(round(abs(out - real)) for out, real in zip(output, correct_answer)),'/7',f' on turn {game_turn}\n')
            turns += 1
            game_turn += 1
